...
* **🛡️ 3중 스팸 방지 보안 시스템 (Anti-Spam System)**:
  1. **Honeypot 필드**: 화면에 보이지 않는 봇 감지용 숨김 트랩 필드로 자동 스패밍 기계 차단.
  2. **60초 재전송 쿨다운 (Rate Limiting)**: `localStorage` 기반 동일 사용자의 60초 내 연속 도배 방지.
  3. **초고속 제출 & 글자 수 검증**: 1.5초 미만 매크로 자동 제출 차단 및 5자 이상 메시지 검증.
