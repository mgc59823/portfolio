...
* **🛡️ 4중 스팸 방지 보안 시스템 (Anti-Spam System)**:
  1. **🔒 보안 퀴즈 (Math Captcha)**: 무작위 산술 덧셈 퀴즈(`7 + 4 = ?`)를 통과해야만 전송 가능.
  2. **⏱️ 3분(180초) 쿨다운 제한**: 전송 시도 즉시 3분간 재전송 차단.
  3. **🚫 동일 메시지 중복 제출 차단**: 캐시 비교를 통해 동일 내용의 연쇄 연속 발송 100% 방지.
  4. **🍯 Honeypot 트랩 필드**: 화면에 보이지 않는 봇 감지용 숨김 필드로 자동화 스패머 차단.
